...
const AboutHeroSection = () => {
  ...
  const [AboutHeroDataArr, setAboutHeroDataArr] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchAboutHeroData = async () => {
      ...
      try {
        ...
        const ApiResponse = await axios.get(`${BE_URL}/aboutHeroSection`);
        
        if (ApiResponse.status === 200) {

          setAboutHeroDataArr(ApiResponse.data.data);

        } else {
          console.error("Error fetching About Hero data:", ApiResponse.statusText);
        }
      } catch (error) {
          ...
          AboutHeroDataArr && AboutHeroDataArr.map((val, idx) => {
            ...
            return (
              ...
              <div className={`flex  py-10 max-w-screen-xl mx-auto md:gap-10 gap-0 ${isMobile ? "flex-col" : isEven ? "flex-row" : "flex-row-reverse"}`} key={idx}>
                ...
                <motion.div className={`content flex flex-col justify-around ite  ms-center  ${isMobile ? "px-5" : isEven ? "pl-20" : "pr-20"}  gap-10 xl:w-1/2 w-full`} initial={{ opacity: 0, y: -50 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.6, delay: idx * 0.2 }}
                  viewport={{ once: true }}>


                  {/* <p className="text-[1rem] text-gray-700 font-[600] text-justify">{val.description}</p> */}

                  <div className="text-[1rem] text-gray-700 font-[600] text-justify space-y-4">
                ...
                <motion.div className="about-img xl:w-1/2 w-full flex justify-center" initial={{ opacity: 0, y: 50 }}
                  ...
                  <img
                    src={`${BE_URL}/Images/AboutImages/AboutHeroSection/${val.image}`}
                    alt="about hero"
                    className={`h-100 rounded-2xl lg:p-0 p-2 object-cover`} />
