  ...
  useEffect(() => {
    ...
    const FetchContactData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${BE_URL}/contact-section-address`);
        ...
        <div className="container max-w-screen-xl mx-auto md:px-10 px-5 py-10 flex flex-col gap-10 ">
          ...
          <div className="flex md:flex-row flex-col justify-between gap-10 text-center">
            ...
            {contactData.map((item, index) => (
              <div key={index} className="flex flex-row gap-5 p-10 relative items-start group md:w-1/2 w-full rounded-2xl shadow-md/50 transition-all duration-300 ease-in">
                <div className="absolute top-0 left-0 w-full h-0 bg-orange-color -z-10 transition-all duration-500 ease-in-out group-hover:h-full rounded-2xl"></div>
                {/* <MdEmail size={30} className="text-orange-color mt-2 group-hover:text-white" /> */}
                <div className="flex flex-col gap-5 justify-center w-full">
                  <h2 className="text-black font-semibold md:text-[1.5rem] text-[1.3rem] group-hover:text-white">{item.title}</h2>
                  <p className="text-gray-500 font-semibold text-[1rem] group-hover:text-white">{item.address}</p>
                </div>
              </div>
