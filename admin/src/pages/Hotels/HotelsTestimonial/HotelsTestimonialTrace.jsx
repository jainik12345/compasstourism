...
const HotelsTestimonialTrace = () => {
  ...
  const fetchTrashedTestimonialsByHotel = async (hotelId) => {
    if (!hotelId) {
      setData([]);
      return;
    }
    ...
    try {
      const res = await axios.get(`${BE_URL}/hotelTestimonial/trashed`);
      const filtered = res.data.data.filter((item) => item.hotel_id == hotelId);
      setData(filtered);
    } catch (err) {
      console.error("Error fetching trashed testimonials:", err);
      setData([]);
    } finally {
  ...
  const handleHotelChange = (e) => {
    ...
    setPage(1);
    fetchTrashedTestimonialsByHotel(e.target.value);
  };

  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/hotelTestimonial/restore/${id}`);
      setShowRestorePopup(true);
      setTimeout(() => {
        setShowRestorePopup(false);
        fetchTrashedTestimonialsByHotel(selectedHotel);
      }, 2500);
    } catch (err) {
    ...
    <div className="p-4 rounded-xl bg-white">
      ...
      {loadingTestimonials ? (
        ...
        <p className="text-center text-gray-500">
          No trashed testimonial data found for selected hotel.
        </p>
      ) : (
...
export default HotelsTestimonialTrace;
