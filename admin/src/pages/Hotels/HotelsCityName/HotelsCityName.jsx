...
const HotelsCityName = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`${BE_URL}/hotelCityName`);
  ...
  const handleAddClick = () => {
    navigate("/hotels-city-name/insert");
  };

  const handleTraceClick = () => {
    navigate("/hotels-city-name/trace");
  };

  const handleEditClick = (row) => {
    navigate("/hotels-city-name/update", { state: { rowData: row } });
  };

  const handleDelete = async (id) => {
    try {
      await axios.delete(`${BE_URL}/hotelCityName/${id}`);
