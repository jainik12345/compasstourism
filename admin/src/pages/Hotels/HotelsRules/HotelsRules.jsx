...
const HotelsRules = () => {
  ...
  // Fetch rules for selected hotel
  const fetchRules = async (hotelId) => {
    try {
      const res = await axios.get(`${BE_URL}/hotelRules/${hotelId}`);
      // Each row: { id, hotel_id, check_in, check_out, rooms, floors, rules: JSON }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${BE_URL}/hotelRules/${id}`);
  ...
  useEffect(() => {
    if (selectedHotelId) fetchRules(selectedHotelId);
  }, [selectedHotelId]);
  ...
  return (
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-between mb-4">
        <Trace onClick={() => navigate("/hotels-rules/trace")} />
        <Add
          text="Add Rules"
          width="w-[220px]"
          onClick={() => navigate("/hotels-rules/insert")}
        />
      </div>
                  ...
                  <ol className="list-decimal ml-5">
                    {Array.isArray(row.rules) &&
                      row.rules.map((rule, rIdx) => (
                        <li key={rIdx}>
                          <span className="font-semibold">{rule.heading}</span>
