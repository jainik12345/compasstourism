...
const HotelsAboutTrace = () => {
  ...
  const fetchTrashedAboutByHotel = async (hotelId) => {
    if (!hotelId) {
      setData([]);
      return;
    }
    ...
    try {
      const res = await axios.get(`${BE_URL}/hotelAbout/trashed`);
      const filtered = res.data.data.filter((item) => item.hotel_id == hotelId);
      setData(filtered);
    } catch (err) {
      console.error("Error fetching trashed about data:", err);
      setData([]);
    } finally {
  ...
  const handleHotelChange = (e) => {
    ...
    setPage(1);
    fetchTrashedAboutByHotel(e.target.value);
  };

  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/hotelAbout/restore/${id}`);
      setShowRestorePopup(true);
      setTimeout(() => {
        setShowRestorePopup(false);
        fetchTrashedAboutByHotel(selectedHotel);
      }, 2500);
    } catch (err) {
  ...
  return (
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-between mb-4">
        <h2 className="text-left font-semibold text-xl">Hotels About Trace</h2>
      ...
      {loadingAbout ? (
        ...
        <p className="text-center text-gray-500">
          No trashed about data found for selected hotel.
        </p>
      ) : (
...
export default HotelsAboutTrace;
