...
const HotelsFacilitiesTrace = () => {
  ...
  // Fetch trashed facilities, then filter by hotelId on frontend
  const fetchTrashedFacilitiesByHotel = async (hotelId) => {
    if (!hotelId) {
      setData([]);
      return;
    }
    ...
    try {
      const res = await axios.get(`${BE_URL}/hotelFacilities/trashed`);
      // Filter trashed facilities by hotel_id (assumes each item has hotel_id)
      ...
      const formatted = filtered.map((item) => ({
        ...item,
        image: item.image
          ? `${BE_URL}/Images/HotelImages/HotelFacilities/${item.image}`
          : "",
      }));
      setData(formatted);
    } catch (err) {
      console.error("Error fetching trashed facilities:", err);
      setData([]);
    } finally {
  ...
  const handleHotelChange = (e) => {
    ...
    setPage(1);
    fetchTrashedFacilitiesByHotel(e.target.value);
  };

  // Restore facility data by id
  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/hotelFacilities/restore/${id}`);
      setShowRestorePopup(true);
      setTimeout(() => {
        setShowRestorePopup(false);
        fetchTrashedFacilitiesByHotel(selectedHotel);
      }, 2500);
    } catch (err) {
    ...
    <div className="p-4 rounded-xl bg-white">
      ...
      {loadingFacilities ? (
        ...
        <p className="text-center text-gray-500">
          No trashed facilities found for selected hotel.
        </p>
      ) : (
...
export default HotelsFacilitiesTrace;
