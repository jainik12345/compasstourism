...
const HotelsFacilitiesDetailsTrace = () => {
  ...
  // Fetch trashed facilities details, then filter by hotelId on frontend
  const fetchTrashedFacilitiesDetailsByHotel = async (hotelId) => {
    if (!hotelId) {
      setData([]);
      return;
    }
    ...
    try {
      const res = await axios.get(`${BE_URL}/hotelFacilitiesDetails/trashed`);
      // Filter trashed details by hotel_id (assumes each item has hotel_id)
      ...
      console.error("Error fetching trashed details:", err);
      setData([]);
    } finally {
  ...
  const handleHotelChange = (e) => {
    ...
    setPage(1);
    fetchTrashedFacilitiesDetailsByHotel(e.target.value);
  };

  // Restore facility data by id
  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/hotelFacilitiesDetails/restore/${id}`);
      setShowRestorePopup(true);
      setTimeout(() => {
        setShowRestorePopup(false);
        fetchTrashedFacilitiesDetailsByHotel(selectedHotel);
      }, 2500);
    } catch (err) {
    ...
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-between mb-4">
        <h2 className="text-left font-semibold text-xl">
          Hotels Facilities Details Trace
        </h2>
      ...
      {loadingFacilities ? (
        ...
        <p className="text-center text-gray-500">
          No trashed facilities details found for selected hotel.
        </p>
      ) : (
