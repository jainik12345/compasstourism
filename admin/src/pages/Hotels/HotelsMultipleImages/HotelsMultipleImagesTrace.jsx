...
const HotelsMultipleImagesTrace = () => {
  ...
  // Fetch trashed multiple images (all), then filter by hotelId on frontend
  const fetchTrashedImagesByHotel = async (hotelId) => {
    if (!hotelId) {
      setData([]);
      return;
    }
    ...
    try {
      const res = await axios.get(`${BE_URL}/hotelMultipleImages/trashed`);
      // Filter trashed images by hotel_id (assuming each item has hotel_id)
  ...
  const handleHotelChange = (e) => {
    ...
    setPage(1);
    fetchTrashedImagesByHotel(e.target.value);
  };

  // Restore image data by id
  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/hotelMultipleImages/restore/${id}`);
      setShowRestorePopup(true);
      setTimeout(() => {
        setShowRestorePopup(false);
        fetchTrashedImagesByHotel(selectedHotel);
      }, 2500);
    } catch (err) {
    ...
    <div className="p-4 rounded-xl bg-white">
      ...
      {loadingImages ? (
        ...
        <p className="text-center text-gray-500">
          No trashed images found for selected hotel.
        </p>
      ) : (
...
export default HotelsMultipleImagesTrace;
