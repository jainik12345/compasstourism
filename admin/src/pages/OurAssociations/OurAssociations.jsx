...
const OurAssociations = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get(`${BE_URL}/ourAssociations`);
      ...
      const formatted = responseData.map((item) => ({
        ...
        images: item.images.map(
          (filename) => `${BE_URL}/Images/OurAssociations/${filename}`
        ),
      }));
      setData(formatted);
    } catch (err) {
  ...
  //   try {
  //     await axios.delete(`${BE_URL}/ourAssociations/${id}`);
  ...
  return (
    <div className="p-4 rounded-xl bg-white">
      ...
      <div className="flex justify-end mb-4">
        {/* <Trace onClick={() => navigate("/our-associations/trace")} /> */}
        <Add
          text="Add Association Images"
          width="w-[250px]"
          onClick={() => navigate("/our-associations/insert")}
        />
      </div>
                ...
                <TableCell className="text-left">
                  <div className="flex space-x-4">
                    <button
                      ...
                      onClick={() =>
                        navigate("/our-associations/update", {
                          state: { rowData: row },
                        })
                      }
                    >
